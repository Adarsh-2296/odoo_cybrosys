...
class PosCategory(models.Model):
    ...
    def _compute_is_discount_limit(self):
        for record in self:
            record.is_discount_limit = False
            shops = self.env['pos.config'].search([])
            category = shops.mapped('pos_category_ids.id')
            if record.id in category:
                record.is_discount_limit = True

    @api.model
    def _load_pos_data_fields(self, config_id):
        """
        Adds the 'discount_limit' field to the list of fields loaded into the POS.
        """
        data = super()._load_pos_data_fields(config_id)
        data += ['is_discount_limit','discount_limit']
        return data
